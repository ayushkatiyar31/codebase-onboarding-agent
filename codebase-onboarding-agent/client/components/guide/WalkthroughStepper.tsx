...
export default function WalkthroughStepper({ owner, repoName, onFileSelect }: WalkthroughStepperProps) {
    ...
    if (error || steps.length === 0) {
        return (<div className="flex flex-col items-center justify-center h-full gap-3 text-gray-500">
        <AlertCircle size={28}/>
    ...
    return (<div className="flex flex-col h-full p-6 gap-6">
      <div className="flex items-center gap-2 justify-center">
        {steps.map((_, i) => (<button key={i} onClick={() => setCurrentIndex(i)} className={`h-1.5 rounded-full transition-all duration-300
                        ${i === currentIndex ? 'w-8 bg-blue-500' : 'w-1.5 bg-gray-700 hover:bg-gray-600'}`} aria-label={`Go to step ${i + 1}`}/>))}
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-5 max-w-lg mx-auto text-center">
        ...
        <button onClick={() => onFileSelect?.(step.filePath)} className="flex items-center gap-2 px-4 py-2 bg-gray-800 hover:bg-gray-700
      ...
      <div className="flex items-center justify-between shrink-0">
        <button onClick={() => setCurrentIndex(i => Math.max(0, i - 1))} disabled={isFirst} className="flex items-center gap-1.5 px-4 py-2 text-sm text-gray-400
                     hover:text-white disabled:opacity-30 disabled:cursor-not-allowed
                     transition-colors">
          <ChevronLeft size={16}/> Previous
        </button>

        <button onClick={() => onFileSelect?.(step.filePath)} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg text-sm
                     text-white font-medium transition-colors">
          Open this file
        </button>

        <button onClick={() => setCurrentIndex(i => Math.min(steps.length - 1, i + 1))} disabled={isLast} className="flex items-center gap-1.5 px-4 py-2 text-sm text-gray-400
                     hover:text-white disabled:opacity-30 disabled:cursor-not-allowed
                     transition-colors">
