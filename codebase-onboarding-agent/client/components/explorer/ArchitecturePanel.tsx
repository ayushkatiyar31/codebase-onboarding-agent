...
import {
  ...
  GitBranch, FolderOpen, Zap, AlertTriangle,
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 text-gray-400 p-8">
        ...
        <div className="text-center">
          <p className="text-white font-medium mb-1">Analysing architecture...</p>
          <p className="text-sm text-gray-500">{status || 'Preparing context...'}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-6 overflow-y-auto h-full">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={async () => {
            await fetch(`${apiBase}/api/analysis/${owner}/${repoName}/cache`, {
              method: 'DELETE',
            });
            retry();
          }}
      ...
      {result.architecturePattern && (
        <Section title="Architecture Pattern">
          <div className="flex items-start gap-3">
            <span className="px-2.5 py-1 bg-blue-600/20 text-blue-300 rounded-md text-sm font-medium border border-blue-500/20">
        ...
        <Section title="Tech Stack">
          <div className="flex flex-wrap gap-2">
            {result.techStack.map((tech, i) => (
              <TechBadge key={i} name={tech.name} role={tech.role} category={tech.category} />
            ))}
          </div>
        </Section>
      )}
      ...
      {result.dataFlow && (
        <Section title="How Data Flows">
          <p className="text-gray-300 text-sm leading-relaxed">{result.dataFlow}</p>
        </Section>
      )}
          ...
          <div className="flex flex-col gap-2">
            {result.entryPoints.map((ep, i) => (
              <div key={i} className="flex items-start gap-2 text-sm">
                <code className="text-blue-400 font-mono text-xs bg-gray-800
        ...
        <Section title="Key Directories" icon={<FolderOpen size={14} />}>
          <div className="flex flex-col gap-2">
            {result.keyDirectories.map((dir, i) => (
              <div key={i} className="flex items-start gap-2 text-sm">
                <code className="text-purple-400 font-mono text-xs bg-gray-800
                                  px-2 py-0.5 rounded whitespace-nowrap mt-0.5">
                  {dir.path}/
                </code>
          ...
          <ol className="flex flex-col gap-2">
            {result.firstFilesToRead.map((file, i) => (
              <li key={i} className="flex items-start gap-3 text-sm">
                <span className="text-gray-600 font-mono text-xs mt-0.5 w-4 flex-shrink-0">
                  {i + 1}.
                </span>
                <div>
                  <code className="text-green-400 font-mono text-xs">{file.path}</code>
      ...
      {result.gotchas?.length > 0 && (
        <Section title="Watch Out For" icon={<AlertTriangle size={14} className="text-yellow-400" />}>
...
const TechBadge = ({ name, role, category }: { name: string; role: string; category: string }) => {
  const colours = CATEGORY_COLOURS[category] ?? CATEGORY_COLOURS.other;

  return (
    <div className={`group relative flex items-center gap-1.5 px-2.5 py-1
