...
export default function DependencyGraph({ owner, repoName, onFileSelect }: DependencyGraphProps) {
    ...
    if (error) {
        ...
        <button onClick={fetchGraph} className="flex items-center gap-2 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm transition-colors">
    ...
    return (<div className="h-full w-full relative">
      <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeClick={handleNodeClick} onNodeDoubleClick={handleNodeDoubleClick} fitView minZoom={0.1} maxZoom={2} proOptions={{ hideAttribution: true }}>
        <Background color="#1f2937" gap={16}/>
        <Controls className="bg-gray-800 border-gray-700"/>

    <MiniMap className="bg-gray-900" nodeColor={(node) => node.id === selectedNode ? '#3b82f6' : '#374151'} maskColor="rgba(0,0,0,0.6)"/>
      </ReactFlow>
